...
def main():
    ...
    model = train_model(dataloader, 4, len(idx2word), embeddings, device)
    torch.save(model.state_dict(), 'all_checkpoint_16step2.pth')
    # Test
    trained_model = Simple_LSTM(len(idx2word),256,embeddings).to(device)
    trained_model.load_state_dict(torch.load('all_checkpoint_32step.pth'))
